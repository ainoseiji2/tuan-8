...
namespace Nhan
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            StreamWriter sw = new StreamWriter("D:\\emaildata1.txt");
            sw.Write(s);
            sr.Close();
            sw.Close();
            XmlDocument xmldoc = new XmlDocument();
            xmldoc.Load("D:\\emaildata1.txt");
            ...
            foreach (XmlNode node in xmldoc.SelectNodes(@"/feed/entry"))
            {
                ...
                string time = node.SelectSingleNode("modified").InnerText;
                Console.WriteLine("Nguoi Gui :{0} \nTieu de :{1} \nNoi dung: {2}\nThoi Gian :{3}", email, title, summary,time);
